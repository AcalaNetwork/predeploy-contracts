...
const generate = async () => {
  ...
  for (const token of tokens) {
    ...
    const replaced = fileData
      ...
      // The currencyid is u8, it needs to be converted to uint256, and it needs to satisfy `v[15] = currencyId`, so add `00000000000000000000000000000000`at the end.
      .replace(/uint256 private constant _currencyId = 0xffff;/, `uint256 private constant _currencyId = ${"0x" + currencyId.toString(16) + "00000000000000000000000000000000"};`)
