...
const OracleABI =
  require("../artifacts/contracts/oracle/Oracle.sol/Oracle.json").abi;
...
describe("Prices", () => {
  ...
  it("getPrice works", async () => {
    ...
    await feedValues(
      "DOT",
      BigNumber.from(15).mul(BigNumber.from(10).pow(18)).toString()
    );
    expect(await prices.getPrice(DOT)).to.equal(
      BigNumber.from(15).mul(BigNumber.from(10).pow(18)).toString()
    );

    await feedValues(
      "DOT",
      BigNumber.from(16).mul(BigNumber.from(10).pow(18)).toString()
    );
    expect(await prices.getPrice(DOT)).to.equal(
      BigNumber.from(16).mul(BigNumber.from(10).pow(18)).toString()
    );

    expect(await prices.getPrice(AUSD)).to.equal(
  ...
  it("ignores invalid address as CurrencyId::erc20", async () => {
    // not system contract
    expect(
      await prices.getPrice("0x1000000000000000000000000000000000000000")
    ).to.equal(0);
    // Zero address
