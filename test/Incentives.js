  ...
  describe("Incentive Tests", function () {
    ...
    describe("getIncentiveRewardAmount", function () {
      it("works", async function () {
        const updateRewards = provider.api.tx.sudo.sudo(
          provider.api.tx.incentives.updateIncentiveRewards([
            [{ Loans: { Token: "ACA" } }, [[{ Token: "ACA" }, 100]]],
          ])
        );
        await send(updateRewards, testPairs.alice.address);
        const rewardAmount = await instance.getIncentiveRewardAmount(
          PoolId.Loans,
          ACA,
          ACA
        );
      ...
      it("reverts when input bad PoolId Value", async function () {
        await expect(instance.getIncentiveRewardAmount(2, ACA, ACA)).to.be
          .reverted;
      });
    });
      ...
      it("works", async function () {
        ...
        const updateRewards = provider.api.tx.sudo.sudo(
          provider.api.tx.incentives.updateDexSavingRewards([
            [
              { Dex: { DexShare: [{ Token: "ACA" }, { Token: "AUSD" }] } },
              Rate,
            ],
          ])
        );
    ...
    describe("getClaimRewardDeductionRate", function () {
      it("empty storage returns 0", async function () {
        const rewardDeductionRate = await instance.getClaimRewardDeductionRate(
          PoolId.Dex,
          LP_DOT_AUSD
        );
    ...
    describe("getPendingRewards", function () {
      it("empty storage returns 0", async function () {
        ...
        const pendingRewards = await instance.getPendingRewards(
          [ACA, AUSD, LDOT],
          PoolId.Loans,
          ACA,
          userAddress
      ...
      it("returns values", async function () {
        ...
        await expect(
          instance.connect(wallet).depositDexShare(LP_ACA_AUSD, 1_000_000_000)
        )
          .to.emit(instance, "DepositedShare")
          .withArgs(wallet.getAddress, LP_ACA_AUSD, 1_000_000_000);

        for (let i = 0; i < 10; i++) {
          await provider.api.rpc.engine.createBlock(
            true /* create empty */,
            true /* finalize it*/
          );
        }
        ...
        await expect(
          instance.connect(wallet).withdrawDexShare(LP_ACA_AUSD, 1_000_000_000)
        )
        ...
        await expect(
          instance.connect(wallet).claimRewards(PoolId.Dex, LP_ACA_AUSD)
        )
