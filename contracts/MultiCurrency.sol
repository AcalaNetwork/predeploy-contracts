...
library MultiCurrency {
    function totalSupply(uint256 currencyId) internal view returns (uint256) {
        ...
        assembly {
            if iszero(
                staticcall(gas, 0x0000000000000000001, input, 0x20, output, 0x20)
            ) {
                revert(0, 0)
            }
        }
    ...
    function balanceOf(uint256 currencyId, address account) internal view returns (uint256) {
        ...
        assembly {
            if iszero(
                staticcall(gas, 0x0000000000000000001, input, 0x40, output, 0x20)
            ) {
                revert(0, 0)
            }
        }
    ...
    function transfer(uint256 currencyId, address sender, address recipient, uint256 amount) internal {
        ...
        assembly {
            if iszero(
                staticcall(gas, 0x0000000000000000001, input, 0x80, 0x00, 0x00)
            ) {
                revert(0, 0)
            }
        }
